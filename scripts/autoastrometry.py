...
def writeconfigfile(satlevel=65000.):
    configs='''
#-------------------------------- Catalog ------------------------------------
...
DETECT_THRESH    7              # <sigmas> or <threshold>,<ZP> in mag.arcsec-2
ANALYSIS_THRESH  7              # <sigmas> or <threshold>,<ZP> in mag.arcsec-2
...
#PHOT_AUTOPARAMS  4, 6        # MAG_AUTO parameters: <Kron_fact>,<min_radius>
...
def getcatalog(catalog, ra, dec, boxsize, minmag=8.0, maxmag=-1, maxpm=60.):
    ...
    if maxmag == -1:
        ...
        if catalog == 'gsc2': maxmag = 18.0

    
    if (catalog =='ub2' or catalog=='sdss' or catalog=='tmc' or catalog=='gsc2'):
        usercat = 0
        racolumn = 1
        deccolumn = 2
        magcolumn = 6
...
overwrite=False, outfile='', saturation=-1, quiet=False):
    ...
    if pixelscale > 0 and pa > -360:
       ...
       try:
            equinox = float(h.get('EQUINOX', 2000.)) #If RA and DEC are not J2000 then convert
       except ValueError:
            equinox = 2000.
    ...
    h["CRVAL1"] = raoffset + cra
    h["CRVAL2"] = decoffset + cdec
    
    #h.update("ASTRMTCH", catalog)
    try:
       oldcat = h['ASTR_CAT']
       h["OLD_CAT"] = (oldcat, "Earlier catalog oldcat")
    except:
       pass
