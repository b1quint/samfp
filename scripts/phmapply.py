...
def main():
    ...
    FSR = round(FSR / abs(sample)) # From BCV to Channels
    if v:
...
def shiftSpectrum(spec, dz, nPoints=100):
    ...
    # Re-sample spectrum
    newZ    = np.linspace(z[0], z[-1], z.size * nPoints)
    newSpec = spline(newZ)

    # Add padded borders
    newSpec = np.append(np.zeros(dzPoints), newSpec)
    newSpec = np.append(newSpec, np.zeros(dzPoints))

    # Shifting spectrum
    newSpec = np.roll(newSpec, int(dzSign * dzPoints))
...
def shift_spectrum(spec, dz, fsr=-1, sample=1.0, n_points=100):
    """
    @param spec: 1D np.array representing a spectrum to be shifted.
